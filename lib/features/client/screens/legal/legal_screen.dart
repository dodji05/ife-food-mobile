      ...
      body: data.when(
        ...
        data: (page) => page == null
          ...
          : ListView(padding: const EdgeInsets.all(24), children: [
              Text(page['title'] ?? '', style: const TextStyle(fontFamily: 'Nunito', fontSize: 22, fontWeight: FontWeight.w800, color: AppColors.nearBlack, height: 1.3)),
              const SizedBox(height: 16),
              Text(page['content'] ?? '', style: const TextStyle(fontFamily: 'Nunito', fontSize: 15, color: AppColors.darkGrey, height: 1.7)),
              const SizedBox(height: 40),
              Text('Version ${page['version'] ?? '1.0'}', style: const TextStyle(fontFamily: 'Nunito', fontSize: 11, color: AppColors.grey)),
            ]),
      ),
    );
  }
