...
class ProChatScreen extends ConsumerStatefulWidget {
  ...
  const ProChatScreen({super.key, required this.orderId});
...
class _Msg {
  ...
  factory _Msg.fromJson(Map<String, dynamic> j) {
    ...
    return _Msg(
      ...
      senderName: [firstName, name].where((s) => s.isNotEmpty).join(' ').trim().isEmpty
          ? 'Inconnu'
          : [firstName, name].where((s) => s.isNotEmpty).join(' ').trim(),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: isMine ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
          if (!isMine) ...[
            Text(
              msg.senderName,
              style: const TextStyle(fontFamily: 'Nunito', fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.primary),
            ),
